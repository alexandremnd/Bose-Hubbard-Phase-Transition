...
template <typename _Iterator> class Tqdm : public MyIteratorWrapper<_Iterator>
{
  ...
  public:
    ...
    /** constructors
     */
    explicit Tqdm(_Iterator begin, _Iterator end) : TQDM_IT(begin), e(end), self()
    {
        self.total = size_t(end - begin);
    }

    explicit Tqdm(_Iterator begin, size_t total) : TQDM_IT(begin), e(begin + total), self()
    {
        self.total = total;
    }
    ...
    template <typename _Container, typename = typename std::enable_if<!std::is_same<_Container, Tqdm>::value>::type>
    ...
    {
        if (this->get() == e)
            throw std::out_of_range("exhausted"); // TODO: don't throw, just double total
    ...
    {
        ((Tqdm const &)*this)._incr();
    }
};

template <typename _Iterator, typename _Tqdm = Tqdm<_Iterator>> _Tqdm tqdm(_Iterator begin, _Iterator end)
{
    return _Tqdm(begin, end);
}

template <typename _Iterator, typename _Tqdm = Tqdm<_Iterator>> _Tqdm tqdm(_Iterator begin, size_t total)
{
    return _Tqdm(begin, total);
}

template <typename _Container, typename _Tqdm = Tqdm<typename _Container::iterator>> _Tqdm tqdm(_Container &v)
{
    return _Tqdm(v);
}

template <size_t N, typename T, typename _Tqdm = Tqdm<T *>> _Tqdm tqdm(T (&tab)[N])
{
    return _Tqdm(tab, N);
}

template <typename SizeType = int> using RangeTqdm = Tqdm<RangeIterator<SizeType>>;
template <typename SizeType> RangeTqdm<SizeType> range(SizeType n)
{
    return RangeTqdm<SizeType>(RangeIterator<SizeType>(n), RangeIterator<SizeType>(n));
}
template <typename SizeType> RangeTqdm<SizeType> range(SizeType start, SizeType end)
{
    return RangeTqdm<SizeType>(RangeIterator<SizeType>(start, end), RangeIterator<SizeType>(start, end));
}
template <typename SizeType> RangeTqdm<SizeType> range(SizeType start, SizeType end, SizeType step)
{
    return RangeTqdm<SizeType>(RangeIterator<SizeType>(start, end, step), RangeIterator<SizeType>(start, end, step));
}
