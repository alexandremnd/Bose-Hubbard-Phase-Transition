...
#define IS_WIN
#elif !defined IS_NIX && (defined(unix) || defined(__unix) || defined(__unix__) || defined(__linux__) ||               \
                          defined(__FreeBSD__) || defined(__APPLE__) || defined(__MACH__))
#define IS_NIX
#endif
...
class MyIteratorWrapper
    : public std::iterator<std::forward_iterator_tag, typename std::iterator_traits<_Iterator>::value_type>
{
  ...
  public:
    ...
    template <class Other> bool operator!=(const MyIteratorWrapper<Other> &rhs) const
    {
        return p != rhs.p;
    }
    ...
    const _Iterator &get() const
    {
        return p;
    }
    // TODO: const _Iterator &get() const { return p; }, etc ...
    //
    void swap(MyIteratorWrapper &other) noexcept
    {
        std::swap(p, other.p);
    }
    ...
    // }
    template <typename = typename std::is_pointer<_Iterator>> explicit operator bool() const
    {
        return p != nullptr;
    }
};

template <typename _Iterator, typename _MyIteratorWrapper = MyIteratorWrapper<_Iterator>>
_MyIteratorWrapper myIteratorWrapper(_Iterator x)
{
    return _MyIteratorWrapper(x);
}

template <typename IntType = int> class RangeIterator : public std::iterator<std::forward_iterator_tag, IntType>
{
  private:
  ...
  public:
    ...
    RangeIterator(IntType start, IntType total) : current(start), total(total), step(1)
    {
    }
    RangeIterator(IntType start, IntType total, IntType step) : current(start), total(total), step(step)
    {
    }
    IntType &operator*()
    {
...
template <class Node> AtomicList<Node>::AtomicList() : meta(static_cast<Node *>(&meta), static_cast<Node *>(&meta))
{
}
